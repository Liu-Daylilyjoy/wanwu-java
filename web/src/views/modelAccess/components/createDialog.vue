    ...
    >
      <template slot="title">
        <div class="dialog-title-wrapper">
          ...
          <span class="dialog-desc" v-if="isXunFeiProvider()">
            {{ $t('modelAccess.hint.xunfei') }}
          </span>
        </div>
      </template>
      <el-form
      ...
      >
        ...
        <el-form-item v-if="showXunFeiAuth()" label="AppID" prop="appId">
          <el-input
        ...
        >
          <el-input
            ...
            :placeholder="
              $t('common.hint.apiKey') +
              (typeObj.apiKey[providerConfigKey()] || '--')
            "
            :disabled="!allowEdit"
          ></el-input>
        </el-form-item>
        ...
        >
          <el-input
            ...
            :title="
              ...
              (typeObj.inferUrl[
                `${createForm.modelType}_${providerConfigKey()}`
              ] ||
                typeObj.inferUrl[providerConfigKey()] ||
                '--')
            "
            :placeholder="
              $t('common.hint.inferUrl') +
...
export default {
  ...
  data() {
    const validateUrls = (rule, value, callback) => {
      const reg =
        /^(http|ftp|https|ws|wss):\/\/[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&:/~\+#]*[\w\-\@?^=%&/~\+#])?$/;
  ...
  methods: {
    ...
    isXunFeiProvider() {
      ...
      return (
        key === XUNFEI ||
        key === PROVIDER_OBJ[XUNFEI] ||
        name === PROVIDER_OBJ[XUNFEI] ||
        /讯飞|iFlytek/i.test(`${key}${name}`)
      );
    },
    providerConfigKey() {
      return this.isXunFeiProvider() ? XUNFEI : this.provider.key;
    },
    defaultModelIcon() {
      return this.isXunFeiProvider()
        ? PROVIDER_IMG_OBJ[XUNFEI]
        : this.defaultLogo;
    },
    isMultiModal() {
    ...
    showXunFeiAuth() {
      return this.isXunFeiProvider() && this.createForm.modelType === ASR;
    },
    showVision() {
      return (
    ...
    getModelIdList() {
      fetchModelIdList({
        provider: this.providerConfigKey(),
    ...
    setDefaultInferUrl() {
      const providerKey = this.providerConfigKey();
      const defaultUrl =
        this.typeObj.inferUrl[`${this.createForm.modelType}_${providerKey}`] ||
        this.typeObj.inferUrl[providerKey];
      if (defaultUrl) {
        // OpenAI-API-compatible 供应商不自动设置推理 URL
        this.createForm.endpointUrl =
          providerKey === OPENAI_API ? '' : defaultUrl;
      }
    },
