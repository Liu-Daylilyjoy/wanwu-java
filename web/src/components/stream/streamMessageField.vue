                  ...
                  >
                    <section class="reasoning-area-content">
                      <div
                        ...
                        class="chunk_stable"
                        @click="$refs.imagePreview.handleImageClick($event)"
                        v-html="normalizeAnswerHtml(chunk)"
                      ></div>
                      <div
                        v-if="n.activeReasoning"
                        class="chunk_active"
                        @click="$refs.imagePreview.handleImageClick($event)"
                        v-html="normalizeAnswerHtml(n.activeReasoning)"
                      ></div>
                    </section>
                  </div>
                </template>
...
import { normalizeHtmlImageSrc, normalizeUrlEntities } from '@/utils/url';
...
export default {
  ...
  methods: {
    ...
    normalizeAnswerHtml(data) {
      return normalizeHtmlImageSrc(data);
    },
    handleScroll(e) {
    ...
    unwrapCodeImages(html) {
      ...
      const replacer = (match, inner) => {
        ...
        if (!m) return match;
        const url = normalizeUrlEntities(m[2]);
      ...
      return normalizeHtmlImageSrc(out);
    },
    /**
