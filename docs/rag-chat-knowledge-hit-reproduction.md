...
This slice connects the Java RAG chat path to the local Java knowledge hit API and forwards configured rerank model IDs plus Go-style image attachment context into that local request contract. It does not reproduce the full Go RAG runtime yet. The remaining work includes real file parsing/indexing, vector/keyword/rerank retrieval execution parity, model generation, conversation memory, graph RAG behavior, callback-producing parser/indexer workers, object storage lifecycle, non-image attachment parsing, and streaming token generation from a provider model.
