...
The document knowledge hit path now follows the Go BFF and proto response shape for `prompt`, `searchList`, `score`, and `useGraph`. It runs a deterministic local match over enabled imported document segments, child segments, and request `docInfoList` upload fallbacks, using exact substring matches first and token-overlap fallback for natural questions. It fills frontend card fields (`title`, `snippet`, `knowledgeName`, `contentType = text`, `childContentList`, `childScore`, `score`, and `rerankInfo` / `rerank_info`), honors score/threshold inputs, ranks all local candidates by score before applying `topK`, emits local rerank evidence when `rerankMod=rerank_model` or `rerankModelId` is present, appends a `contentType=graph` local graph card when `useGraph=true` and the knowledge base graph switch is enabled, and ignores disabled parent segments. This gives `/knowledge/hit` a real frontend-visible loop while the true RAG/vector/rerank integration remains a later replacement.
