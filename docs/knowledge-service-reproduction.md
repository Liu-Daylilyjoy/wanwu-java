...
The document path derives searchable segments from request content, uploaded UTF-8 data, base64 PDF/DOC/DOCX/XLSX content, or URL metadata, then builds and persists embeddings for every parent segment. Segment create/update, reimport, and config-driven re-splitting rebuild the affected vector index synchronously. Custom splitter and parent-child settings affect chunking, while selected-document ZIP exports remain synchronous. PPTX/HTML and richer multimodal parsing, object storage, and asynchronous parser/index callbacks remain pending.
...
- Go-equivalent MinIO object lifecycle, richer Office/multimodal parsing, asynchronous reimport workers, MinIO-backed export files, and parser/indexer/report workers that emit callbacks.
