...
- Workflow run returns the raw workflow output JSON and workflow/chatflow upload returns the raw signed URL text, matching Go's direct `ctx.Writer.Write` / `ctx.String` behavior instead of the normal BFF response envelope.
