...
- Doc Center indexes Markdown files from `wanwu-service-bff/src/main/resources/static/manual` for menu, search, entry, and Markdown content, with deterministic local Markdown seeds kept as a fallback.
...
- The Doc Center uses a lightweight Java classpath Markdown index rather than Go's full `static/manual` asset directory and riot search engine. The current repository intentionally includes lightweight Markdown seeds; copying the full 99MB Go manual asset tree remains a separate asset-porting slice.
