...
| `v1/rag.go` | 10 | Partially covered; RAG app draft/create/update/config/copy/delete/list/publish/version, AG-UI draft/published chat shell, and multipart upload compatibility covered for both user-domain and service-domain frontend access; real RAG retrieval/generation still missing |
...
| --- | ---: | --- |
| `AppService` | 41 | Partially covered for assistant CRUD/config, publish/version/OpenURL, legacy `/use/model/api/v1` assistant aliases, app keys/API keys, persisted local assistant resource bindings, RAG app lifecycle/chat shell, Workflow app lifecycle/import/export/run shell, Chatflow lifecycle/application shell, Safety guard management shell, and frontend statistics dashboard compatibility; real RAG orchestration, real Workflow/Chatflow engine execution, legacy model-use/CUBM runtime, real statistics aggregation, and runtime sensitive-word interception missing |
...
15. RAG: draft/publish/chat/copy/version and knowledge integration. Management lifecycle, frontend chat shell, and user/service-domain upload response shapes are covered; retrieval/search-list generation remains next.
