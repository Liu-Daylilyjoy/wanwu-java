...
| `v1/workflow.go` | 20 | Partially covered; Workflow appspace create/list/copy/import/export/delete, generic publish/version integration, local run shell, assistant workflow select, `/workflow/api` parameter/use/openapi-schema compatibility, and avatar upload route covered; full visual workflow editor engine and Coze execution still missing |
...
11. RAG: draft/publish/chat/copy/version and knowledge integration. Management lifecycle, frontend chat shell, and upload response shape are covered; retrieval/search-list generation remains next.
12. Assistant full surface: Workflow app lifecycle is now covered as a local shell; continue with MCP tools, custom tools, skills, multi-agent, templates, and deeper select endpoints.
13. Guest/callback/openapi: public API, API usage metrics runtime, callback compatibility.
14. WGA/general agent and sandbox integrations.
