...
| `v1/common.go` | 48 | Partially covered; app-key, app-list, model selects, local file upload/check/merge/clean/delete/direct-upload compatibility, and doc-center shell covered; user profile/password/avatar writes and several select helpers still missing |
...
| `IAMService` | 49 | Development login, frontend permission split, IAM user/role/org read compatibility, platform custom setting readback, and OAuth app management covered; persisted user/org/role/OAuth writes and Operate custom config storage still missing |
...
13. RAG: draft/publish/chat/copy/version and knowledge integration. Management lifecycle, frontend chat shell, and upload response shape are covered; retrieval/search-list generation remains next.
14. Assistant full surface: Workflow app lifecycle is now covered as a local shell; continue with deeper runtime orchestration and select endpoints.
15. Guest/callback/openapi: public API, API usage metrics runtime, callback compatibility.
16. WGA/general agent and sandbox integrations remain out of the current exposed frontend scope unless explicitly reintroduced.
