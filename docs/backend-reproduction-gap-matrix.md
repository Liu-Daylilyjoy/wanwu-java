...
12. RAG: draft/publish/chat/copy/version and knowledge integration. Management lifecycle, frontend chat shell, and upload response shape are covered; retrieval/search-list generation remains next.
13. Assistant full surface: Workflow app lifecycle is now covered as a local shell; continue with deeper runtime orchestration and select endpoints.
14. Guest/callback/openapi: public API, API usage metrics runtime, callback compatibility.
15. WGA/general agent and sandbox integrations remain out of the current exposed frontend scope unless explicitly reintroduced.
