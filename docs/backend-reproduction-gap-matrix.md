...
10. RAG: draft/publish/chat/copy/version and knowledge integration. Management lifecycle, frontend chat shell, and upload response shape are covered; retrieval/search-list generation remains next.
11. Assistant full surface: Workflow app lifecycle is now covered as a local shell; continue with MCP tools, custom tools, skills, multi-agent, templates, and deeper select endpoints.
12. Guest/statistics/callback/openapi: public API, API usage metrics, callback compatibility.
13. WGA/general agent and sandbox integrations.
