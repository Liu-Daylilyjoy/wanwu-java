...
- Workflow and Chatflow now have persisted local app lifecycles and frontend-compatible import/export shells. Workflow run snapshots, Go-style process readback, schema-aware outputs, node declared-output mapping, Go-template numeric node basics including selector/input nodes, intent classification, HTTP request, knowledge query, text-like document parse, local tool-node output compatibility, JSON deserialize, and JSON-to-CSV code pattern, semantic JSON serialize, `block-output` input references, prior-node output propagation, deterministic node trace output, and text/handle/object-style conditional edge execution are persisted for development verification, while the actual visual editor engine, advanced branch semantics, Coze-compatible runtime, arbitrary code/tool/MCP/Skill node execution, binary Office/PDF deep parsing in AppService, node validation, and advanced Workflow/Chatflow marketplace/template flows are still missing.
