...
- Template square compatibility: assistant and Workflow template list/detail/recommend/download data plus Workflow template download counts are backed by `app_templates`, with the BFF also merging a read-only bundle generated from the 30 local Go Workflow templates.
