...
- Assistant chat now supports configured OpenAI-compatible model generation as an aggregated answer before AppService persistence, then enriches from local knowledge hits when configured. RAG chat still returns deterministic local answers after validating draft/published app existence and enriching from local knowledge hits. Real vector/rerank retrieval, reasoning frames, token-by-token assistant streaming, tool/MCP/multi-agent orchestration, and provider-specific adapters remain future slices.
