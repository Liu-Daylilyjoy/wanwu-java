...
- Workflow app lifecycle: create/list/copy/import/export/delete, Go-style export attachment headers, generic app publish/unpublish/version list/version update/rollback, MySQL-persisted local run snapshots with schema-aware output, assistant workflow select from real created workflows, workflow tool select/action/tool-box compatibility backed by resource-center tools, `/workflow/api/workflow/parameter`, `/workflow/api/api/workflow/use`, `/workflow/api/workflow/openapi_schema`, and `/api/bot/upload_file` avatar upload compatibility.
