...
- Workflow app lifecycle: create/list/copy/import/export/delete, Go-style export attachment headers, generic app publish/unpublish/version list/version update/rollback, MySQL-persisted local run snapshots with schema-aware output, Go-style process readback from persisted run records, node `outputs/output` declaration mapping with template variable rendering, Go-template numeric node basics (`1` start, `2` end, `3` LLM, `5` code JSON-to-CSV, `8` selector, `15` string concat, `22` intent classification, `30` supplemental input, `32` variable merge, `45` HTTP request, `1006` knowledge query, `1007` local document generate, `1008` document parse for text-like files, `1009` tool-node local `result.content` output, `1059` JSON deserialize, plus semantic JSON serialize nodes), `sourceNodeID/targetNodeID` edges, `block-output` literal/ref input resolution, prior-node output propagation for later nodes, deterministic node trace output, and text/handle/object-style conditional edge execution, assistant workflow select from real created workflows, workflow tool select/action/tool-box compatibility backed by resource-center tools, `/workflow/api/workflow/parameter`, `/workflow/api/api/workflow/use`, `/workflow/api/api/workflow_api/get_process`, `/workflow/api/workflow/openapi_schema`, and `/api/bot/upload_file` avatar upload compatibility.
...
- Workflow and Chatflow now have persisted local app lifecycles and frontend-compatible import/export shells. Workflow run snapshots, Go-style process readback, schema-aware outputs, node declared-output mapping, Go-template numeric node basics including selector/input nodes, intent classification, HTTP request, knowledge query, local document generation, text-like document parse, local tool-node output compatibility, JSON deserialize, and JSON-to-CSV code pattern, semantic JSON serialize, `block-output` input references, prior-node output propagation, deterministic node trace output, and text/handle/object-style conditional edge execution are persisted for development verification, while the actual visual editor engine, advanced branch semantics, Coze-compatible runtime, arbitrary code/tool/MCP/Skill node execution, object-storage document lifecycle, binary Office/PDF deep parsing in AppService, node validation, and advanced Workflow/Chatflow marketplace/template flows are still missing.
