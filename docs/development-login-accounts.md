...
| --- | --- | --- | --- |
| `admin` | `dev-token` | `permission`, `permission.user`, `permission.org`, `permission.role`, `app`, `app.agent`, `api_key`, `api_key.api_key_management` | Verify permission-management read views, Agent, and API Key menus without exposing unreproduced modules. |
...
Frontend permissions are intentionally narrower than `web/src/router/constants.js` while the Java reproduction is still incomplete. Unreproduced modules stay hidden until their backend slices are implemented, tested, and verified through Docker Compose. Permission management currently covers the read-only user, role, organization, role-select, and role-template views; write operations are still a later IAM persistence slice. The ontology agent menu has been removed from this reproduction scope.
