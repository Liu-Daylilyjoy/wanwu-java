...
The frontend and public OpenAPI RAG chat paths also mirror the Go runtime's use of the configured model ID far enough for real development loops: BFF reads the draft/published RAG `modelConfig.modelId`, calls the Java OpenAI-compatible upstream path with `stream:true`, aggregates the provider deltas, and passes that answer into AppService before knowledge/QA enrichment, safety output replacement, and chat-record persistence. That upstream request now preserves `needHistory=true` RAG history as ordered OpenAI-compatible `user/assistant` messages before the current question; frontend RAG image uploads ending in `.png`, `.jpg`, or `.jpeg` are sent to the configured model as OpenAI-compatible `image_url` content parts on the current user question. The enrichment step accepts both frontend `knowledgebases/config` and Go proto `perKnowledgeConfigs/globalConfig` shapes, preserving `graphSwitch`, metadata-filter request fields, weighted-score vs rerank-model mode, the configured `rerankConfig.modelId` / `qaRerankConfig.modelId`, and Go-style image attachments for the Java knowledge hit request context. The Java knowledge-service compatibility layer now applies Go-style `metadata_filtering_conditions` during local document hit execution, uses exact substring plus token-overlap fallback scoring for local document and QA recall, emits local `rerankInfo` / `rerank_info` evidence when rerank mode is requested, appends local `contentType=graph` evidence when `useGraph=true` and either the knowledge graph switch or request-level `knowledgeList[].graphSwitch` is enabled, persists QA `metaDataList`, applies Go-style `metadataFilteringConditions` during local QA hit execution, and returns QA hit metadata through `meta_data` / `metaData`. Frontend RAG SSE now emits Go-style QA and knowledge start/search-list AG-UI custom events before the text content. If the model config is absent, inactive, or unreachable, AppService keeps the deterministic local fallback so development Docker remains usable offline.
...
This slice reproduces the Go RPC boundary, state transitions, frontend/OpenAPI configured-model answer loop, frontend image upload propagation into OpenAI-compatible model calls, rerank model ID propagation plus local rerank evidence fields, graph flag propagation plus local graph evidence, metadata-filter request shaping plus local document/QA metadata filtering, and image attachment request shaping into local hit requests, not the full Go runtime. True retrieval/rerank execution orchestration, provider token-by-token RAG streaming, exact stream chunk sequencing, file parser/indexer callbacks, object-storage lifecycle, non-image attachment parsing, and provider-specific adapter behavior remain later RAG runtime work.
