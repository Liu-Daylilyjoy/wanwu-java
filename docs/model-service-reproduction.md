...
- `/callback/v1/model/{modelId}/chat/completions`, `/embeddings`, and `/rerank` now mirror the Go callback path at a minimum usable level: when `ModelService#getModel` returns a configured model with `endpointUrl` or `inferUrl` plus a non-development `apiKey`, the BFF forwards non-stream OpenAI-compatible JSON requests to the matching upstream endpoint and returns the upstream JSON body. Missing model service, missing credentials, development redacted keys, and upstream failures still fall back to the deterministic local response so Docker development remains self-contained.
...
- Callback inference parity beyond non-stream OpenAI-compatible chat/embeddings/rerank: streaming chat, multimodal request conversion, provider-specific adapters, token statistics, and callback auth/signature checks remain missing.
