...
- `wanwu-service-model` implements:
  ...
  - `/model/validate-thinking` now follows the Go BFF validation shape for LLM imports: when a real `endpointUrl`/`apiKey` is supplied it posts OpenAI-compatible `/chat/completions` requests with `enable_thinking=true` and `enable_thinking=false`, requiring reasoning content in the enabled response and no reasoning content in the disabled response; Docker development placeholder keys keep the self-contained success fallback.
