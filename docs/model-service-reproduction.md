...
- Callback inference parity beyond OpenAI-compatible chat stream/non-stream with image URL data conversion plus embeddings/rerank JSON proxying: broader multimodal conversion, provider-specific adapters, token statistics, and callback auth/signature checks remain missing.
