...
- `/callback/v1/model/{modelId}/chat/completions` now mirrors the Go callback path at a minimum usable level: when `ModelService#getModel` returns a configured model with `endpointUrl` or `inferUrl` plus a non-development `apiKey`, the BFF forwards a non-stream OpenAI-compatible chat request to the upstream `/chat/completions` endpoint and returns the upstream JSON body. Missing model service, missing credentials, development redacted keys, and upstream failures still fall back to the deterministic local response so Docker development remains self-contained.
...
- Callback inference parity beyond non-stream OpenAI-compatible chat completions: streaming chat, embeddings, rerank, multimodal request conversion, provider-specific adapters, token statistics, and callback auth/signature checks remain missing.
