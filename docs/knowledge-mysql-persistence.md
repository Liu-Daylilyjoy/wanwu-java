...
- `KnowledgeServiceImpl` loads and saves one JSON snapshot containing the current frontend-compatible knowledge, tag, keyword, splitter, document, segment, metadata, permission, and QA-pair state.
...
- Unit tests cover snapshot upsert, startup reload, restored knowledge/tag/keyword/doc/QA state, and sequence continuation.
...
- Implement real QA import/export parsing, vector/keyword/rerank retrieval, keyword extraction/sync, graph generation, report generation, and RAG query integration.
