...
- `KnowledgeServiceImpl` loads and saves one JSON snapshot containing the current frontend-compatible knowledge, tag, splitter, document, segment, metadata, permission, and QA-pair state.
...
- Unit tests cover snapshot upsert, startup reload, restored knowledge/tag/doc/QA state, and sequence continuation.
...
- Normalize the snapshot into Go-equivalent tables such as `knowledge_base`, `knowledge_doc`, `knowledge_qa_pair`, `knowledge_tag`, `knowledge_tag_relation`, `knowledge_splitter`, and `knowledge_permission`.
- Implement real file byte ingestion, MinIO object lifecycle, document parsing, chunking, vector indexing, reimport, child segments, export records, and async status callbacks.
- Implement real QA import/export parsing, vector/keyword/rerank retrieval, keyword extraction, graph generation, report generation, and RAG query integration.
